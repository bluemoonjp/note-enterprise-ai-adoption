...
- **本プロジェクトの見解**: したがって「機密情報は入力禁止」という一律の入力可否だけでは、RAG・
  ...
  | 軸 | 制御手段 | 状態 |
  | --- | --- | --- |
  | 機密度 | 2.2節の3区分表 | 既存の仕組みで対応済み |
  | 処理環境 | 3節の層A/B/C | 既存の仕組みで対応済み |
  | 利用者権限（誰が使えるか） | RBAC（4節） | 既存の仕組みで対応済み |
  | 出力先・参照範囲 | コネクタ許可制（4節） | 既存の仕組みで対応済み |
  | 利用目的（何のために使うか） | [01](01-導入判断.md)1.1節の用途別許可／条件付き／禁止（粗い判定） | 専用の制御機構は現時点で無し。今後の検討課題 |

  5軸のうち4軸は、小規模IT部門の設定（4節）と情報区分（2節）の組み合わせで既にカバーできる。
  ...
  **別の軸**であり、両者を混同しない（4節のRBACは利用者権限軸の制御であって、利用目的軸の代替では
  ない）。今回のパイロットは対象人数・用途を限定した小規模IT部門限定であるため、利用目的軸の粗さに
  よる残余リスクは限定的であり、統制が整わないまま見切り発車するという意味ではない。ワークスペース
  分離等でのさらなる高度化は、パイロット後の拡大局面での検討課題とする。「何を入力してよいか」という
  単一の判定基準に頼るのではなく、複数の統制を組み合わせて初めて、一律禁止に頼らずに済む設計になる。
