...
- 「機密情報は一律入力禁止」という発想は、単発のチャット利用には使えても、RAG・エージェント・
  情報統合（[12](12-AI活用の全体像.md)の②〜④段階、小規模な部署内統合を含む）とは構造的に
  両立しない。統制の単位を「入力してよい情報か」から「機密度×処理環境×利用者権限×利用目的×
  出力先」の組み合わせへ移す必要がある（2.4節）。
...
- 都度「顧客A」「案件X」のように固有名詞を伏せてから入力する運用は、単発のプロンプトには使えても、
  **同一人物・同一案件の同一性そのものを壊してしまう**ため、継続的な情報統合の基盤としては
  機能しない。これは小規模な部署内統合（例: 部門内のチケット・問い合わせの横断把握）でも同様に
  当てはまり、対象範囲が全社か部署内かという規模の大小では解消しない構造的な制約である。
- **本プロジェクトの見解**: したがって「機密情報は入力禁止」という一律の入力可否だけでは、RAG・
  エージェント・情報統合という利用形態そのものと両立しない。この場合の統制は、「入力してよい
  情報か否か」という単一の軸ではなく、**情報の機密度 × 処理環境 × 利用者権限 × 利用目的 × 出力先**
  という複数軸の組み合わせで設計する必要がある。実務上は、経路（3節の層A/B/C）で処理環境を、
  RBAC（4節）で利用者権限を、コネクタ許可制（4節）で参照範囲・出力先を、それぞれ制御する形になる。
  「何を入力してよいか」という単一の判定基準に頼るのではなく、複数の統制を組み合わせて初めて、
  一律禁止に頼らずに済む設計になる。
