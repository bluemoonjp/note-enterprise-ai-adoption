...
「どの情報ならAIに入力してよいか」は、契約形態だけでは決まらない。この記事は情報分類・入力可否・
保持/削除・越境移転・DPA・調達確認という切り口で整理する。組織の責任分担は[04](04-ガバナンス.md)、
製品別の詳細比較は[06](06-製品プラン比較.md)に譲る。対象は日本の法人契約
...
- 情報は「AI専用の新しい区分」を作らず、既存の情報区分に**入力可否を紐づける**形で整理する。
  マスキングを使えば、多くの「条件付き」案件は安全に「原則OK」側へ動かせる（2節）。
- Team/Businessの時点で、学習不使用・SSO・コネクタ許可制・DPAは対応が確認できている。利用が
  RAG・エージェント（[12](12-AI活用の全体像.md)の②③段階）に広がる場合は、これに加えて
  **機密度×処理環境×利用者権限×利用目的×出力先**という複数軸の組み合わせで統制する設計にする
  （2.4節）。
- 通常業務は法人SaaS（層A）、機微度が高い領域は自社クラウド経由（層B: Bedrock/Azure/Vertex）、
  という**階層構成**が実務的（3節）。エージェント固有のリスクである**権限越境**への対策は
  コネクタ許可制と最小権限設計が本丸になる（4節、詳細は [08](08-IT部門向けAIエージェント.md)）。
...
これらの組み合わせで、単発の質問応答（チャット、[12](12-AI活用の全体像.md)の①段階）を中心とする
利用は十分に運用できる。

ただし、利用が社内文書に基づく回答・資料生成（RAG、[12](12-AI活用の全体像.md)②段階）や、複数
...
- **本プロジェクトの見解**: したがって「機密情報は入力禁止」という一律の入力可否だけでは、RAG・
  ...
  利用目的 × 出力先**という複数軸の組み合わせで設計する必要がある。実務上の対応は次の通り。

  | 軸 | 制御手段 | 状態 |
  | --- | --- | --- |
  | 機密度 | 2.2節の3区分表 | 本プロジェクトの分類運用として対応済み |
  | 処理環境 | 3節の層A/B/C | 本プロジェクトの経路設計として対応済み |
  | 利用者権限（誰が使えるか） | RBAC（4節） | プラン依存。Enterprise系は対応、パイロットで想定するTeam/Businessは対応可否が未確認（[06](06-製品プラン比較.md)2.2節） |
  | 出力先・参照範囲 | コネクタ許可制（4節） | Team/Businessで対応確認済み |
  | 利用目的（何のために使うか） | [01](01-導入判断.md)1.1節の用途別許可／条件付き／禁止（粗い判定） | 専用の制御機構は現時点で無し。今後の検討課題 |

  5軸のうち機密度・処理環境・出力先の3軸は、小規模IT部門の設定（3節・4節）と情報区分（2節）の
  組み合わせで既にカバーできる。**利用者権限（RBAC）は、Enterprise系プランでは対応するが、
  パイロットで想定する層A（Team/Business、[06](06-製品プラン比較.md)）では対応可否が未確認**であり、
  [10](10-パイロット運営.md)1.3節の方針に従って契約前にベンダーへ確認したうえで、対応していなければ
  パイロット段階の必須条件にはしない（Team/Businessでも確認済みのSSO・コネクタ許可制で代替統制する）。
  残る「利用目的」軸は、[01](01-導入判断.md)の粗い用途判定以上の専用機構を持たない点を正直に
  開示するが、これは「利用者権限（誰が使えるか）」を制御するRBAC（4節の「モデル・機能のRBAC」）とは
  **別の軸**であり、両者を混同しない（4節のRBACは利用者権限軸の制御であって、利用目的軸の代替では
  ない）。今回のパイロットは対象人数・用途を限定した小規模IT部門限定であるため、利用者権限軸・
  利用目的軸それぞれの粗さによる残余リスクは限定的であり、統制が整わないまま見切り発車するという
  意味ではない。RBACの本格運用・ワークスペース分離等でのさらなる高度化は、パイロット後の拡大局面
  （Enterprise化）での検討課題とする。「何を入力してよいか」という単一の判定基準に頼るのではなく、
  複数の統制を組み合わせて初めて、一律禁止に頼らずに済む設計になる。
