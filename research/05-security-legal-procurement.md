...
- 通常業務は法人SaaS（層A）、機微度が高い領域は自社クラウド経由（層B: Bedrock/Azure/Vertex）、
  という**階層構成**が実務的（3節）。
- 小規模IT部門が最初に締めるべき設定は、禁止事項の作文より先に、**SSO+SCIM・監査ログ・データ保持期間・
  コネクタ許可制**（4節）。
...
「統制の効く小集団（小規模IT部門）に限定して使う」という論理が必要になる（[06](06-vendor-plans.md) で
プランごとの機能差を詳細比較）。
...
- SSO+SCIM・監査ログ・データ保持期間・コネクタ許可制は、小規模IT部門限定の導入であっても
